...
#'
#' @export
NOmap <- function(Bathleg = TRUE, Border = TRUE, Trim = 45, Grats = FALSE, straight = FALSE, land = TRUE, fronts = FALSE, frontcols = c("hotpink", "orchid", "plum"), bordercol = c("white", "black"), borderwidth = 2, gratcol = "grey70") {
    ## data
    NOmap_data <- NULL
    Bathy <- NULL
    data("NOmap_data", package = "NOmap", envir = environment())
    data("Bathy", package = "NOmap", envir = environment())
    ...
    yy <- c(Trim, seq(from = Trim + 1, to = 90, by = 15))
    grat <- graticule::graticule(xx, yy, proj = raster::projection(Bathy))
    gratlab <- graticule::graticule_labels(lons = 180,lats = -rev(c(-45, -30, -60, -75)), xline = 180, yline = 15, proj = raster::projection(Bathy))

    ## crop bathy raster depending on legend yes or no
    q <- ifelse(Bathleg, Trim+borderwidth+11, Trim+borderwidth)
    Bathy <- raster::trim(latmask(Bathy, latitude = q, southern = FALSE))
    ...
    buf <- NOmake_buf(Trim+borderwidth, proj = out$projection)

    if (land) {
      xland <-sf::st_as_sf(NOmap_data$continent)
...
#      out$plot_sequence <- c(out$plot_sequence, "fronts")
    ...
    structure(out, class = "NOmap")
}

#' @method plot NOmap
#' @export
plot.NOmap <- function (x, y, ...) {
    print(x)
    invisible()
}
