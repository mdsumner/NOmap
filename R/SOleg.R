...
SOleg <-function(x=NULL,
  ...
  if(type=="continuous" && !is.null(ticks) && !is.null(breaks)&& length(breaks)!= ticks ){
    stop("Number of ticks and breaks do not match. You do not need to use ticks if you ahve breaks")
  }

  if(type=="continuous" && !inherits(breaks, "numeric") || !inherits(breaks,"integer")){
